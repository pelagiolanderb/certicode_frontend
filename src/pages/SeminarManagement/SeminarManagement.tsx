...
const ManageSeminarPage = () => {
  ...
  const handleParticipant = async (seminar_id, user_id) => {
    ...
    try {
      ...
      console.log("Joined seminar:", result.seminar);
      alert(
        `You have successfully joined the seminar: ${result.seminar.title}`
      );
    } catch (error) {
      alert("Failed to join seminar.");
    }
  };
